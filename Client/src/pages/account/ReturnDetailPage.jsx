    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>
      ...
      <div className="bg-white shadow-sm p-5 sm:p-7 mb-6 border border-neutral-200/80 rounded-lg flex justify-between items-start flex-wrap gap-4">
        <div className="space-y-2 max-w-xl">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className={`text-xs font-bold px-3 py-1 rounded-full border shadow-2xs ${STATUS_COLORS[returnItem.status] || 'bg-gray-100'}`}>
          ...
          <p className="text-xs sm:text-sm text-neutral-500 leading-relaxed">
            ...
            {orderNumber && (
              <>
                {' '}· Order{' '}
                <Link to={`/account/orders/${orderId}`} className="font-bold text-neutral-900 hover:text-brand-gold hover:underline">
                  {orderNumber}
                </Link>
          ...
          <div className="flex items-start sm:items-center gap-4">
            ...
            <div className="space-y-1">
              <span className="text-[11px] font-bold uppercase tracking-wider text-purple-900 flex items-center gap-1.5">
                <Calendar size={13} className="text-purple-600" /> Scheduled Doorstep Pickup
              </span>
              ...
              <p className="text-xs text-purple-900/80 leading-relaxed max-w-xl">
                Our logistics partner will arrive on this date to collect the parcel. Please keep the item securely packed in its original box with all tags intact.
              </p>
            </div>
          </div>
      ...
      {returnItem.status === 'REJECTED' ? (
        ...
        <div className="bg-white shadow-sm p-5 sm:p-7 mb-6 border border-neutral-200/80 rounded-lg">
          <div className="flex items-center justify-between gap-3 mb-6 pb-4 border-b border-neutral-100">
            ...
            <span className={`text-[11px] font-bold px-3 py-1 rounded-full border ${STATUS_COLORS[returnItem.status] || 'bg-gray-100'}`}>
              {STATUS_LABELS[returnItem.status] || returnItem.status}
            </span>
          </div>
            ...
            {returnSteps.map((step, i) => {
              ...
              return (
                <div key={step.key} className="flex-1 flex flex-col items-center text-center relative min-w-0">
                  ...
                  {i !== 0 && (
                    <div
                      className={`absolute top-4 right-1/2 w-full h-0.5 -z-0 transition-colors ${
                  ...
                  <div
                    ...
                    } ${isCurrent ? 'ring-4 ring-amber-100 scale-105' : ''}`}
                  >
                    {done ? (
                      <Check size={15} className="stroke-[2.5]" />
                    ) : (
                      <Circle size={8} className="fill-neutral-300" />
                    )}
                  </div>
                  {/* Label */}
                  <p
                    className={`text-xs mt-3 font-medium px-1 leading-snug break-words max-w-[120px] ${
                      done ? 'text-neutral-900 font-bold' : 'text-neutral-400'
                    }`}
                  >
                    {step.label}
                  </p>
          ...
          {/* Mobile Stepper */}
          <div className="block md:hidden space-y-0 pl-2 pt-1">
            {returnSteps.map((step, i) => {
              ...
              return (
                <div key={step.key} className="flex items-start gap-4 relative">
                  {!isLast && (
                    <div
                      className={`absolute left-[15px] top-8 bottom-0 w-0.5 -translate-x-1/2 ${
                  ...
                  <div className="flex-1 min-w-0 pb-7">
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <p className={`text-xs sm:text-sm font-bold ${done ? 'text-neutral-900' : 'text-neutral-400'}`}>
                        {step.label}
                      </p>
                      {isCurrent && (
                        <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 bg-amber-50 text-brand-gold border border-amber-200/80 rounded-full shadow-2xs">
                          Current Status
                        </span>
                      )}
                    </div>
        ...
        <div className="bg-white shadow-sm p-4 sm:p-6 border border-neutral-200/80 rounded-lg flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center gap-2 mb-4 pb-3 border-b border-neutral-100">
              ...
              <h2 className="font-playfair text-sm sm:text-base font-bold text-neutral-900">
                Returned Item & Claim Details
              </h2>
            </div>
            ...
            <div className="flex items-start gap-3.5 sm:gap-4">
              ...
              <div className="flex-1 min-w-0 space-y-1.5">
                <h3 className="font-playfair font-bold text-neutral-900 text-sm sm:text-base leading-snug truncate">
                ...
                <p className="text-xs text-neutral-500 pt-0.5">
                  Ordered Item Qty: <span className="font-semibold text-neutral-800">{returnItem.orderItem?.quantity || returnItem.quantity}</span>
                </p>
              </div>
            </div>
            ...
            <div className="mt-3.5 p-3 bg-amber-50/60 border border-amber-200/70 rounded-lg flex items-center justify-between text-xs">
              ...
              <div className="text-right">
                <span className="text-[10px] text-neutral-500 uppercase tracking-wider block font-semibold">Refund Value</span>
                <span className="text-base font-bold text-brand-gold font-mono">
          ...
          <div className="p-3.5 bg-neutral-50/80 rounded-lg border border-neutral-200/70 space-y-2">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <span className="px-2.5 py-1 bg-white text-amber-900 border border-amber-300 rounded-lg text-xs font-semibold shadow-2xs">
                {REASON_LABELS[returnItem.reason] || returnItem.reason}
              </span>
            </div>
            {returnItem.reasonDetails && (
              <div className="bg-white p-2.5 rounded-lg border border-neutral-200/60 text-xs text-neutral-700 leading-relaxed font-sans">
                <span className="text-neutral-400 font-serif mr-1">“</span>
                {returnItem.reasonDetails}
                <span className="text-neutral-400 font-serif ml-1">”</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white shadow-sm p-4 sm:p-6 border border-neutral-200/80 rounded-lg flex flex-col justify-between space-y-4">
          <div>
            <div className="flex items-center justify-between gap-2 mb-3 pb-3 border-b border-neutral-100">
              ...
              {returnItem.unboxingVideoUrl ? (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-800 border border-emerald-200 font-bold text-xs rounded-full shadow-2xs">
                  <ShieldCheck size={13} className="text-emerald-600 shrink-0" />
                  <span>Video Verified</span>
                </span>
              ) : (
                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-neutral-100 text-neutral-500 border border-neutral-200 text-xs font-semibold rounded-full">
                  <span>No Proof</span>
                </span>
              )}
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-xs pt-1">
                  <span className="text-[11px] text-neutral-500 font-medium">
                    Continuous parcel opening video verification proof
                  </span>
                  {isVideoLink && (
                    <a
                      ...
                      className="text-xs text-brand-gold hover:underline font-semibold inline-flex items-center gap-1 shrink-0 ml-2"
                    >
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-1">
                {returnItem.images.map((imgUrl, idx) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="shrink-0 w-14 h-14 rounded-lg border border-neutral-200 overflow-hidden hover:opacity-80 transition-opacity shadow-2xs"
                  >
      ...
      {returnItem.adminNotes && (
        <div className="bg-blue-50/80 shadow-xs p-5 sm:p-6 mb-6 border border-blue-200/80 rounded-lg">
          ...
          <p className="text-xs sm:text-sm text-blue-900 leading-relaxed font-medium">{returnItem.adminNotes}</p>
        </div>
      )}
