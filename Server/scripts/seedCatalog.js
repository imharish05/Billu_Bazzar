...
const { sequelize, Category, SubCategory, Product, Warehouse, WarehouseStock } = require('../models');
  ...
  try {
    ...
    for (const catData of CATALOG_DATA) {
      ...
      for (const subData of catData.subCategories) {
        ...
        for (const sampleProd of (subData.sampleProducts || [])) {
          // Create Sample Product for this SubCategory
          const prodSlug = `${sampleProd.slug}-prod-${Math.floor(100 + Math.random() * 900)}`;
          const sku = `SKU-${sampleProd.slug.toUpperCase().slice(0, 8)}-${Math.floor(1000 + Math.random() * 9000)}`;

          const [product, created] = await Product.findOrCreate({
            where: { name: sampleProd.prodName },
            defaults: {
              name: sampleProd.prodName,
              slug: prodSlug,
              description: `Experience luxury design and superior durability with the ${sampleProd.prodName}. Authored under ${category.name} > ${subCat.name}.`,
              shortDescription: `Premium ${sampleProd.name} featuring state-of-the-art craftsmanship and luxury aesthetics.`,
              price: sampleProd.price,
              comparePrice: Math.round(sampleProd.price * 1.25),
              currency: 'INR',
              sku: sku,
              stock: 50,
              categoryId: category.id,
              ...
              images: [sampleProd.img],
              defaultProductImage: sampleProd.img,
              gstRate: '18%',
              isActive: true,
